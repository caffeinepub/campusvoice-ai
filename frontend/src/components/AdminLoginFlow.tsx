...
import { Shield, Loader2, ArrowLeft, BarChart3, Users, AlertTriangle, ClipboardList } from 'lucide-react';
...
export default function AdminLoginFlow({ onBack }: AdminLoginFlowProps) {
  const { login, isLoggingIn, isInitializing } = useInternetIdentity();

  const handleLogin = async () => {
    localStorage.setItem(PENDING_ROLE_KEY, 'admin');
    try {
      await login();
    } catch (err) {
  ...
  const benefits = [
    { icon: <ClipboardList className="w-4 h-4" />, text: 'System-wide complaint monitoring' },
    { icon: <BarChart3 className="w-4 h-4" />, text: 'Full analytics and reporting dashboard' },
    { icon: <AlertTriangle className="w-4 h-4" />, text: 'Emergency alert management' },
    { icon: <Users className="w-4 h-4" />, text: 'User management and role assignment' },
  ];
          ...
          <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-8 shadow-2xl">
            ...
            <div className="text-center mb-8">
              ...
              <p className="text-white/60 text-sm leading-relaxed">
                Sign in with your Internet Identity to access the Administrator portal
              </p>
            </div>
            ...
            <div className="space-y-2.5 mb-8">
              {benefits.map((b, i) => (
                <div key={i} className="flex items-center gap-3 text-white/70 text-sm">
                  <div className="w-7 h-7 rounded-lg bg-red-500/15 border border-red-400/20 flex items-center justify-center text-red-300 flex-shrink-0">
                    {b.icon}
                  </div>
                  <span>{b.text}</span>
                </div>
            ...
            <Button
              ...
              className="w-full bg-red-600 hover:bg-red-500 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 gap-2.5 h-12"
            >
