...
import { BookOpen, Loader2, ArrowLeft, BarChart3, Users, Bell, ClipboardList } from 'lucide-react';

interface HODLoginFlowProps {
  onBack: () => void;
}

const PENDING_ROLE_KEY = 'campusvoice_pending_role';

export default function HODLoginFlow({ onBack }: HODLoginFlowProps) {
  const { login, isLoggingIn, isInitializing } = useInternetIdentity();

  const handleLogin = async () => {
    // Store the intended role before triggering Internet Identity login
    localStorage.setItem(PENDING_ROLE_KEY, 'hod');
    try {
      await login();
    } catch (err) {
  ...
  const benefits = [
    { icon: <ClipboardList className="w-4 h-4" />, text: 'Review and manage department complaints' },
    { icon: <BarChart3 className="w-4 h-4" />, text: 'Analytics and reporting dashboard' },
    { icon: <Bell className="w-4 h-4" />, text: 'Emergency alert notifications' },
    { icon: <Users className="w-4 h-4" />, text: 'Department-wide oversight' },
  ];
        ...
        <div className="relative z-10 w-full max-w-md mx-auto px-4 py-12">
          ...
          <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-8 shadow-2xl">
            ...
            <div className="text-center mb-8">
              ...
              <h1 className="text-2xl font-display font-bold text-white mb-2">HOD Login</h1>
              <p className="text-white/60 text-sm leading-relaxed">
                Sign in with your Internet Identity to access the Head of Department portal
              </p>
            </div>
            ...
            <div className="space-y-2.5 mb-8">
              {benefits.map((b, i) => (
                <div key={i} className="flex items-center gap-3 text-white/70 text-sm">
                  <div className="w-7 h-7 rounded-lg bg-amber-500/15 border border-amber-400/20 flex items-center justify-center text-amber-300 flex-shrink-0">
                    {b.icon}
                  </div>
                  <span>{b.text}</span>
                </div>
            ...
            <Button
              ...
              className="w-full bg-amber-500 hover:bg-amber-400 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 gap-2.5 h-12"
            >
